...
当遇到现有标签体系无法涵盖的主题时，**不要随意新增**：

1. 先确认文章的主题是否能被现有标签近似覆盖。
2. 如果确实存在系统性空白（例如未来出现了"CUDA Graph"作为频繁主题），**先在正文中使用当前最接近的标签**，然后在 commit message 或给用户的说明中提示"可能需要新增 `XXX` 标签"。
3. 新增标签需要同步更新本文件的标签体系表格。
