...
- 通常 800–1500 字，3-5 个 H2 章节
- 深度定位：串联多条进展、点出趋势方向，每条更新点到即止，不展开底层机制
- 作者：荔枝不耐思

**Reading（论文阅读 / 技术综述）** — 中等深度
- 主题是对单篇论文或单一技术主题的阅读笔记、解读或综述
- 通常 1500–3000 字，4-7 个 H2 章节
- 深度定位：比 Brief 深入——需要拆解论文/技术的核心机制、关键实验结论、适用边界，但不需要像 Essay 那样做跨系统的原创分析或源码级追溯
- 声音是**带立场的阅读笔记**——不是中立地复述论文，而是以质疑者和翻译者视角审视：这个工作在生产里成不成立？它的真正贡献是什么？边界在哪里？
- 允许更多"站在 X 角度看""这篇论文最值得被记住的地方是……"这类显式视角切换
- 结构与 Essay 一致（HTML anchor 引用、`## 参考资料`）
- 作者：Ethan

**Essay（技术深度分析）** — 最深
- 主题是深度技术分析、架构分析、源码解读、benchmark 综述，有完整的核心论点
- 通常 3000–6000 字，5-11 个 H2 章节
...
> | 版本 | 日期 | 说明 |
> |------|------|------|
> | v1.0 | 2026-03-14 | 初稿，基于 SGLang commit `0de0d74`（2026-03-05），覆盖 RadixAttention 与 HiCache |
> | v1.1 | 2026-05-09 | 修订更新：纳入 HiCache 生产数据、HiSparse 与 ShadowRadix |
```
...
|--------|---------|
| v1.0 | 初稿 |
| v1.1, v1.2, … | 增量更新：新增一节、修订数据、补充案例、修改表述 |
| v2.0 | 框架级重写，或核心判断发生大幅变更 |

**正文标注变更位置**：让读者能快速定位哪些是后续补充，在新增或修订内容的**节开头或段落开头**打标：

- 新增整节：`**（vX.X 新增）**` 放在该节第一段开头
- 更新数据或补充结论：`**（vX.X 更新）**` 放在对应段落开头

```markdown
## 八、HiSparse 与 ShadowRadix
...
**微信公众号版本例外**：公众号阅读环境更依赖视觉锚点。`brief -> 微信公众号` 与 `essay -> 微信公众号技术长文` 都必须加入适量 bold，标记正文里的**核心判断、关键数字、关键机制名和读者应该带走的结论**。Essay 长文每个 H2 通常保留 1-3 处 bold；不要把每段都加粗，也不要只加粗孤立名词。优先加粗完整判断短句，例如：

- ✅ **真正决定 agent 成本的不是上下文窗口大小，而是稳定 prefix 能否持续命中 cache read。**
- ✅ **工具 schema 位于缓存层级最顶端，任何小改动都会向 system 和 messages 级联传播。**
- ❌ **KV cache** 是推理系统的重要机制。
...
- [ ] 如果输出微信公众号版本，题图下方导语是否结合当前热点（模型 / 框架 / coding agent / 硬件窗口），并让读者有继续阅读的理由？
- [ ] 如果输出微信公众号版本，正文是否用适量 bold 标注核心判断、关键数字、关键机制名和主要结论，而不是整篇无视觉重点？
