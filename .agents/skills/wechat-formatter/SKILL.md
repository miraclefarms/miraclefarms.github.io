...
**题图下方导语要连接当前热点**：Essay 长文的导语不能只是 intro 的压缩版。它要把文章主题接到读者正在关注的窗口上，例如 Codex、Claude Code、Cursor Background Agents、DeepSeek、Blackwell、新模型首发、长上下文 agent、远端 coding agent 等，用“当前压力 / 热点窗口 → 本文解释的底层机制 → 为什么现在值得读”的结构制造阅读动机。若显式引入新的当前事件，在参考章节追加对应来源。

**正文必须标注重点**：公众号版需要可扫读的视觉锚点。每个 H2 通常保留 1-3 处 bold，优先加粗核心判断、关键数字、关键机制名和主要结论；不要整段加粗，也不要只加粗孤立名词。
...
- Essay 长文：题图下方导语结合当前热点或当前工程压力，并让读者有继续阅读的理由
- Essay 长文：正文有适量 bold，核心判断、关键数字、关键机制名和主要结论可扫读
