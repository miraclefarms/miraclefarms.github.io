...
![vLLM Engine Architecture](/assets/vllm-origin-evolution-community/vllm-architecture-overview.jpg)
...
![vLLM Scheduler Workflow](/assets/vllm-origin-evolution-community/vllm-scheduler-workflow.jpg)

*图 2：vLLM Scheduler 工作流程与资源调度机制。该图展示了 vLLM 调度器的核心工作流程：Scheduler 从请求队列中获取待处理请求，首先进行 Prefix Cache 命中检测以识别可复用的 KV Blocks；随后进行资源估算，计算新请求所需的 Block 数量；基于可用资源和调度策略（FCFS、Priority 等），Scheduler 做出准入决策——如果资源足够则分配 Blocks 并加入 Running 队列，否则让请求在 Waiting 队列中等待或触发 Preemption。调度器与 KVCacheManager 紧耦合，确保调度决策的正确性和资源利用的最大化（来源：vLLM 官方博客）[1]*
...
![vLLM KVCache Manager](/assets/vllm-origin-evolution-community/vllm-kvcache-manager.jpg)

*图 3：vLLM KVCache Manager 架构与 Block 生命周期管理。该图展示了 vLLM KVCache Manager 的核心架构：BlockPool 统一管理三类状态——Free blocks 队列（可分配的空闲 blocks）、Cached blocks 哈希表（已缓存的前缀 blocks 用于 prefix matching）、Active blocks（当前正在使用的 blocks）。KVCacheManager 提供了核心接口包括 get_computed_blocks（查询前缀命中）、allocate_slots（分配新 slots）、free（释放 blocks）和 get_common_prefix_blocks（统计公共前缀）。allocate_slots 的完整流程包括：检查并清理旧 blocks、复用已有缓存、外部 KV 接入、分配新 slots、提交缓存。这套机制实现了 KV 资源的精细化生命周期管理（来源：vLLM 官方博客）[1]*
...
[1] vLLM 官方博客：Inside vLLM: Anatomy of a High-Throughput LLM Inference System：相对路径 blog/vllm.ai
GitHub：[Inside vLLM: Anatomy of a High-Throughput LLM Inference System](https://blog.vllm.ai/blog/anatomy-of-vllm)

[2] PagedAttention 论文：Efficient Memory Management for Large Language Model Serving with PagedAttention：相对路径 arxiv.org/abs/2309.06180
GitHub：[PagedAttention Paper](https://arxiv.org/abs/2309.06180)

[3] vLLM GitHub 仓库（用于观察项目整体结构、版本演进与目录组织）：相对路径 vllm-project/vllm
GitHub：[vllm-project/vllm](https://github.com/vllm-project/vllm)

[4] SGLang 项目（用于对比分析）：相对路径 sgl-project/sglang
GitHub：[sgl-project/sglang](https://github.com/sgl-project/sglang)

[5] vLLM 官方文档：相对路径 docs.vllm.ai
GitHub：[vLLM Documentation](https://docs.vllm.ai/)

[6] vLLM 官方文档：Prefill-Decode Disaggregation：相对路径 docs.vllm.ai/design
GitHub：[Prefill-Decode Disaggregation](https://docs.vllm.ai/en/latest/design/arch_overview.html)

[7] LMCache 项目（用于生态组件分析）：相对路径 LMCache/LMCache
