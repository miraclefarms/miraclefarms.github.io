...
**加粗（Bold）用于**：关键结论、重要数字、核心发现。目的是让跳读的读者一眼能抓住重点。

**标红（`<span style="color:red">`）用于**：全文最核心的技术关键词，出现频率最高的那个概念。标红是稀缺资源，只给真正的主角，不要滥用。

**斜体（`*`）用于**：术语定义、重要引用、reviewer 评论。区分正文叙述和技术细节。

**具体规则**：
- **加粗**：每篇 essay 控制在 8-12 处以内。主要集中在开篇判断句、核心论点的结论句、表格中的关键数值。
- **标红**：全文只选 3-5 个最核心的词（如一篇讲 PagedAttention 的文章，"PagedAttention"就是核心词；一篇讲 FlashAttention-3 的文章，"FlashAttention-3"和"prefill"可能是核心词），在第一次出现时标红，后续出现不再标。
- **斜体**：技术术语首次定义时可用，产品名、框架名通常不加修饰。
- **数字**：关键性能数字（"3x 压缩率"、"延迟降低 40%"、"吞吐量提升 2 倍"等具体数据）可以加粗，突出数据说服力。
- **避免**：满篇加粗等于没有加粗；同一段落内连续三处以上加粗会失去强调效果。

**实现方式**：
- Markdown 加粗：`**关键结论**`
- Markdown 斜体：`*重要引用*`
- GitHub Pages 支持 HTML：`<span style="color:red">核心词</span>`
- Jekyll 主题如果不支持 HTML 颜色，就只保留加粗，不强求标红

**检查方法**：快速浏览全文，只看加粗和标红部分，应该能还原出文章的核心论点轮廓。
...
- [ ] 格式修饰是否克制？加粗控制在 8-12 处以内，标红只用于 3-5 个最核心词
- [ ] 关键数字（性能指标、压缩比）是否加粗突出？
